...
int window_width = grid_width + 1;
int window_height = grid_height + 1;
...
void *getNeighbours(void *p) {
  ...
  for (int i = d->f_index; i < d->l_index; i++) {
    int neigbours = 0;
    // calculate neigbours
    if (d->cells[modulo((i - grid_width - 1), CELLS_SIZE * CELLS_SIZE)] == 1)
      neigbours++; // top left
    if (d->cells[modulo((i - grid_width + 1), CELLS_SIZE * CELLS_SIZE)] == 1)
      neigbours++; // top right

    if (d->cells[modulo((i - grid_width), CELLS_SIZE * CELLS_SIZE)] == 1)
      neigbours++; // above
    if (d->cells[modulo((i + grid_width), CELLS_SIZE * CELLS_SIZE)] == 1)
      neigbours++; // below

    if (d->cells[modulo((i - 1), CELLS_SIZE * CELLS_SIZE)] == 1)
      neigbours++; // left
    if (d->cells[modulo((i + 1), CELLS_SIZE * CELLS_SIZE)] == 1)
      neigbours++; // right
...
void tick(bool *cells, bool *cells_next) {
  ...
  int step = CELLS_SIZE * CELLS_SIZE /
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    ...
    if (tmp_l_index < CELLS_SIZE * CELLS_SIZE) {
      ...
      data->l_index =
          CELLS_SIZE * CELLS_SIZE - 1; // maximum posible index of array
    }
...
void draw(bool *cells) {
  ...
  int curr_visible_cells = DIV_ROUND_CLOSEST(dm.w, grid_cell_size)*DIV_ROUND_CLOSEST(dm.h ,grid_cell_size);
  if(curr_visible_cells >= CELLS_SIZE * CELLS_SIZE)
    curr_visible_cells = CELLS_SIZE * CELLS_SIZE-1;

  int start_pos = DIV_ROUND_CLOSEST(offset_x,grid_cell_size)+DIV_ROUND_CLOSEST((offset_y*grid_width),grid_cell_size);
  if(start_pos >= CELLS_SIZE * CELLS_SIZE)
    start_pos = CELLS_SIZE * CELLS_SIZE-1;

  for (int i = start_pos; i < curr_visible_cells; i++) {
    if (!isAlive(cells, i)) {
      // Move rect
    ...
    while (SDL_PollEvent(&e)) {
      ...
      case SDL_MOUSEBUTTONDOWN:
        if(e.button.button == SDL_BUTTON_LEFT){
          int index = (int)e.motion.y / grid_cell_size * grid_width +
                      e.motion.x / grid_cell_size;
      ...
      case SDL_MOUSEWHEEL: //DOESNT WORK
        if (e.wheel.y > 0){ // scroll up
          ...
          /*if(grid_cell_size >= dm.h){
            grid_cell_size = dm.h;
            scale = 1.0f;
            break;
          }*/
          grid_cell_size = (int) grid_cell_size*scale;
          scale += 0.5f;
          break;
        }
